...
def _strip_prefix(callback_data: str) -> str:
    prefix = f"{CONFIG['prefix']}_"
...
async def list_items_for_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = await admin_keyboards.get_gallery_for_action_keyboard(
        tenant_id,
        action,
        prefix=CONFIG['prefix'],
        back_callback=f"{CONFIG['prefix']}_hub"
    )
    if not reply_markup:
        await query.answer("No hay ítems para gestionar.", show_alert=True)
        return

    action_text = 'modificar' if action == 'modify' else 'eliminar'
    await query.message.edit_text(
        f"Selecciona el ítem que deseas {action_text}:",
