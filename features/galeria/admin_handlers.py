...
async def confirm_delete_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Suponiendo que get_delete_confirmation_keyboard existe en tu proyecto
    await query.message.edit_text(f"<b>⚠️ ¿Seguro que quieres eliminar?</b>\n<blockquote>{escape_html(item_name)}</blockquote>",
...
async def execute_delete_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await list_items_for_action(type('obj', (object,), {'data': f"{CONFIG['prefix']}_delete_list", 'message': query.message, 'answer': query.answer})(), context)
...
async def list_items_for_reorder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    tenant_id = await get_tenant_id(update, context)
    reply_markup = await admin_keyboards.get_gallery_reorder_keyboard(tenant_id)
    if not reply_markup:
