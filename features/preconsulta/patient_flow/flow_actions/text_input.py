...
async def render(update: Update, context: ContextTypes.DEFAULT_TYPE, node: dict, message_id: int = None):
    ...
    target_message_id = message_id or context.user_data.get('consultation_anchor_message_id') or context.user_data.get('anchor_message_id')

    if not target_message_id:
        logger.error(f"No se pudo encontrar un message_id para editar en el nodo {context.user_data.get('current_node_id')}")
