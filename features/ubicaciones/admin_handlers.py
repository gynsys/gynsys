...
async def list_items_for_reorder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    bot_id = await get_tenant_id(update, context)
    reply_markup = await admin_keyboards.get_locations_reorder_keyboard(bot_id)
    if not reply_markup:
        await query.answer("No hay suficientes ubicaciones para reordenar.", show_alert=True); return
...
async def execute_reorder_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ¡LLAMADA CORREGIDA! ---
    # Ahora usamos el módulo 'content_db' que hemos importado
    success = await content_db.reorder_item(bot_id, CONFIG['table'], item_id, direction)

    if success:
        fake_query = type('obj', (object,), {
            'data': f"{CONFIG['prefix']}_reorder_list",
            'message': query.message,
            'answer': lambda *a, **k: asyncio.sleep(0),
            'edit_message_text': query.message.edit_text,
            'edit_message_reply_markup': query.message.edit_reply_markup
        })()
        fake_update = type('obj', (object,), {
            'callback_query': fake_query,
            'effective_user': update.effective_user,
